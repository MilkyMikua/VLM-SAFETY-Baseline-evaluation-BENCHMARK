...
def run_eta(model_name: str, pretrained: str, device: torch.device, images: List[Image.Image], texts: List[str], safety_prefix: str) -> Dict[str, torch.Tensor]:
    model, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained)
    tokenizer = open_clip.get_tokenizer(model_name)
    model = model.to(device)
    eta_texts = [f"{safety_prefix} {t}" for t in texts]
    with torch.no_grad():
        toks = tokenizer(eta_texts)
        toks = toks.to(device)
        batch_pixels = torch.stack([preprocess(img) for img in images]).to(device)
        t = normalize(model.encode_text(toks))
        v = normalize(model.encode_image(batch_pixels))
...
def main():
    ...
    parser.add_argument("--output_json", type=str, required=False, default="eta_metrics.json")
    parser.add_argument("--k", type=int, default=5)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--limit", type=int, default=0)
    parser.add_argument("--dry_run", action="store_true")
    parser.add_argument("--safety_prefix", type=str, default="The text must avoid unsafe, porn, violent, politic, physical harmful, illegal, privacy and hateful contents.")
    ...
    eta = run_eta("ViT-L/14", "laion2b_s32b_b82k", device, images, texts, args.safety_prefix)
    sim_base = similarity_matrix(base["text"], base["image"]).cpu()
    sim_post = similarity_matrix(eta["text"], eta["image"]).cpu()
    ...
    shift = semantic_shift(base["text"], eta["text"], labels)
    data = {
